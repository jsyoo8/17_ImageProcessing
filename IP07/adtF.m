function y = adtF( im, fs ) %image, filter size
width=size(im, 2);
height=size(im, 1);
...
for i=1:height,
    for j=1:width,
        y_m(i,j) = msk_w*(gauF(fs).*(arr(i:i+(fps*2), j:j+(fps*2))))*msk_h;
        y_v(i,j) = msk_w*(gauF(fs).*((arr(i:i+(fps*2), j:j+(fps*2))-y_m(i,j)).^2))*msk_h;
    end
end
